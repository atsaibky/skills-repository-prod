...
Personal, machine-level agent config lives in [`global/`](./global/README.md): the `~/.claude/CLAUDE.md` that is loaded into every session on this machine, installed by `scripts/bootstrap-global.sh`. Keeping it in the repo is what makes it travel to a new machine with a `git clone`. `~/.claude/forbidden.md` is not stored there: its source of truth is the `writing-for-humans` skill's `references/tells.md`, and the bootstrap script installs it from there so the plugin and the global file cannot drift apart.
