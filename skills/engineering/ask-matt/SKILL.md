...
- **`/ste`** rewrites prose into ASD-STE100 Simplified Technical English, the controlled language of aerospace manuals: capped sentence length, restricted verb forms, and one approved word per meaning. Reach for it for documents a wide or non-native audience has to follow exactly. It is the document-sized version of the register `/wait-what` asks for in a single reply.
- **`/teach`**: learn a concept over multiple sessions, using the current directory as a stateful workspace.
- **`/writing-for-agents`** is the reference for writing documents agents consume: skills, AGENTS.md, pointed-at docs.
- **`/writing-for-humans`** is the same job pointed the other way: prose a **person** reads and judges. It carries a named list of the shapes that mark writing as machine-written, and a fix for each, because "make it sound human" is not an instruction a model can act on. Model-invoked, so it fires on any writing task.
